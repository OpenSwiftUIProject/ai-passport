...
`embed/folotoy` 分支为 FoloToy ESP32-C3 编译 OpenSwiftUI 静态展示配置。
...
平台依赖较重的图元提供小型 Embedded 实现。这是**静态展示子集**，不是桌面版
AttributeGraph/RenderBox 渲染器，也不是完整兼容 SwiftUI 的运行时。

设备开机直接展示 View。页面不新增输入行为；继承的长按 OK 可回到硬件菜单，
其中 `OpenSwiftUI` 项可以重新进入。原计数模型及适配层保留作回归测试。
...
layoutPriority、Spacer、动态布局、State/Observation、动画、SF Symbols 或运行时
图片解码。未实现的 API 在编译时不可用。

C 场景接收器在现有 LVGL 锁下管理对象，最多允许 32 个绘图节点，拒绝非法坐标
及未知资源，复制有长度上限的文字，并在退出时删除场景对象。示例使用 8 个节点。
...
其中电量是明确的 **73% 主机测试数据**，不是设备截图。预览还检查未知资源、节点
上限及 20 次页面生命周期；需先经 IDF 构建解析出 `managed_components/lvgl__lvgl`。
