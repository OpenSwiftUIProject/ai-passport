...
After a successful preview, click Build firmware, then download the matching full.bin or Send to Simulator and Open Simulator. Firmware requires ESP-IDF 5.5.3; the default [online Simulator](https://openswiftuiproject.github.io/FoloToy-Passport-Simulator/) runs in your browser without a local Simulator service. Local and published Playground pages both use the online Simulator; cross-origin handoff uses browser window messages, with no server upload. The local Node Simulator remains optional for networking. See [local setup](LOCAL_COMPILER.md) for the full steps and limits. This workflow does not flash hardware.
