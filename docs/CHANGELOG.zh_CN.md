...
- 修复全新 runner 上的主机与固件 CI，显式安装 Swift 6.3.1、固定版本的 OpenSwiftUI 源码与 ESP-IDF，并与发行构建共用环境准备步骤。
...
- 新增由 Layout 求解驱动的 OpenSwiftUI 2048 滑动、合并与新生动画，加入方块身份、LVGL 对象复用、按实际时间推进的页面时钟及有界输入 FIFO，并补充动画中间帧与中途退出测试。

- 为 2048 场景将有容量上限的 LVGL 池从 24 KB 调至 48 KB；主机预览改用有界分配器，检查满盘文字边界与内存池完整性。

- 新增 OpenSwiftUI 2048 菜单 Demo，包含正常堆叠布局、保留的游戏状态、三键切轴操作、不等待双击窗口的按下／松开输入、纯 Swift 规则测试和真实 LVGL 棋盘／终局预览。
